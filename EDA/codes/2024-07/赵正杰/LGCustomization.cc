...
bool LGCustomization::runLegalization()
  {
    ...
    for (auto* inst : movable_inst_list) {
      ...
      int32_t row_idx = int32_t(inst->get_coordi().get_y()/_row_height);
      int32_t row_avail = INT32_MAX;
      int32_t row_cost = INT32_MAX;
      int32_t cost = placeRow(inst,row_idx,0,false);
      ...
      while(cost == INT32_MAX){
        ...
        if(row_idx+i<_database->get_lg_layout()->get_row_num()){
          cost= placeRow(inst,row_idx+1,1,true);
        ...
        if(row_idx-i>=0){
          cost = placeRow(inst,row_idx-i,1,true);
      ...
      if(row_avail!=INT32_MAX){
        placeRow(inst,row_avail,0,false);
      }
  ...
  void LGCustomization::pickAndSortMovableInstList(std::vector<ipl::LGInstance*>& movable_inst_list)
  {
    ...
    std::sort(movable_inst_list.begin(),movable_inst_list.end(),[](ipl::LGInstance* l_inst,ipl::LGInstance* r_inst){
  ...
  int32_t LGCustomization::placeRow(ipl::LGInstance* inst, int32_t row_idx, bool is_trial, bool is_record_cluster)
  {
    ...
    //这里为什么不用考虑是否方的下的问题呢，因为前面已经确定了所找的row_interval_idx一定是一个
  ...
  //数找到实例在当前行内最接近的区间索引，注意，只在当前行考虑，没有考虑y轴的事情
  int32_t LGCustomization::searchNearestIntervalIndex(std::vector<ipl::LGInterval*>& segment_list, ipl::Rectangle<int32_t>& inst_shape)
  {
  ...
  int32_t LGCustomization::calDistanceWithBox(int32_t min_x, int32_t max_x, int32_t box_min_x, int32_t box_max_x)
  {
    //这个函数确实是直接找到box的距离的，根本不管是到那里的距离
    //在范围内
    ...
    }//压根不再这上面，或者说这里box根本放不下
    else {
      return INT32_MAX;
    }
  }

  int32_t LGCustomization::searchRemainSpaceSegIndex(std::vector<ipl::LGInterval*>& segment_list, ipl::Rectangle<int32_t>& inst_shape,
  ...
  //！从左向右逐渐找到一个适合的cluster进行inst的插入，如果都没有的话则给该inst新建一个cluster进行插入，并把新建的cluster与原有cluster产生连接关系
  //！并且返回目标cluster的地址
 AbacusCluster LGCustomization::arrangeInstIntoIntervalCluster(ipl::LGInstance* inst, ipl::LGInterval* interval)
{
  ...
  void LGCustomization::legalizeCluster(AbacusCluster& cluster)
  {
  ...
  void LGCustomization::mergeWithPreviousCluster(AbacusCluster& cluster, AbacusCluster prev_cluster) {
  ...
  void LGCustomization::mergeWithNextCluster(AbacusCluster& cluster, AbacusCluster next_cluster) {
    cluster.appendCluster(next_cluster);
    cluster.set_back_cluster(next_cluster.get_back_cluster());
  }
  //用于为一个 AbacusCluster 对象安排最小的 x 坐标 (min_x) 
  void LGCustomization::arrangeClusterMinXCoordi(AbacusCluster& cluster)
  {
    
    
    int32_t cluster_x = cluster.get_inst_list()[0]->get_coordi().get_x();;
  ...
  //获取前一个cluster最右边的x值
  int32_t LGCustomization::obtainFrontMaxX(AbacusCluster& cluster)
  {
  ...
  int32_t LGCustomization::obtainBackMinX(AbacusCluster& cluster)
  {
  ...
  bool LGCustomization::checkOverlapWithBox(int32_t min_x, int32_t max_x, int32_t box_min_x, int32_t box_max_x)
  {
    if (max_x > box_min_x && min_x < box_max_x) {
      return true;
    }
  ...
  void LGCustomization::replaceClusterInfo(AbacusCluster& modify_cluster, bool is_record_cluster) {
  ...
  AbacusCluster* LGCustomization::findCluster(std::string cluster_name)
  {
    if (cluster_name == "") {
      return nullptr;
    }
  ...
  void LGCustomization::insertCluster(std::string name, AbacusCluster* cluster)
  {
  ...
  void LGCustomization::deleteCluster(std::string name)
  {
    auto it = _cluster_map.find(name);
    if (it != _cluster_map.end()) {
  ...
  void LGCustomization::updateRemainLength(ipl::LGInterval* interval, int32_t delta)
  {
    int32_t cur_value = _interval_remain_length[interval->get_index()];
    _interval_remain_length[interval->get_index()] = cur_value + delta;
  }

  void LGCustomization::splitTargetInst(ipl::LGInstance* inst, cu_RollbackInfo& rollback_info)
  {
  ...
  bool LGCustomization::runRollback(bool clear_but_not_rollback) {
    if (clear_but_not_rollback) {
      // clear.
      std::stack<cu_RollbackInfo>().swap(_rollback_stack);
      return true;
    }
  ...
  void LGCustomization::deleteTargetIntervalClusters(ipl::LGInterval* interval, std::vector<AbacusCluster>& cluster_list) {
  ...
  void LGCustomization::insertTargetIntervalClusters(ipl::LGInterval* interval, std::vector<AbacusCluster>& cluster_list) {
  ...
  void LGCustomization::insertClusterChainIntoInterval(ipl::LGInterval* interval, std::vector<AbacusCluster*>& cluster_chain) {
    int32_t interval_idx = interval->get_index();
    auto* interval_root = _interval_cluster_root[interval_idx];
  ...
  void LGCustomization::reCalIntervalRemainLength(ipl::LGInterval* interval) {
    int32_t interval_idx = interval->get_index();
  ...
  void LGCustomization::debugIntervalRemainLength(std::string interval_name) {
  ...
  std::string LGCustomization::obtainUniqueClusterName(std::string origin_name){
