...
TetrisCluster::TetrisCluster(std::string name)
    : _name(name),
...
int32_t TetrisCluster::get_max_x()
{
  int32_t max_x = _min_x + _total_width;
  return max_x;
}

void TetrisCluster::clearTetrisInfo()
{
  _weight_e = 0.0;
  _weight_q = 0.0;
  _total_width = 0;
...
void TetrisCluster::insertInstance(ipl::LGInstance* inst)
{
...
void TetrisCluster::appendCluster(TetrisCluster& cluster)
{
...
void TetrisCluster::appendInst(ipl::LGInstance* inst)
{
...
void TetrisCluster::appendInstList(std::vector<ipl::LGInstance*> inst_list){
...
void TetrisCluster::updateTetrisInfo()
{
  clearTetrisInfo();
  for (size_t i = 0; i < _inst_list.size(); i++) {
...
int32_t TetrisCluster::obtainInstIdx(ipl::LGInstance* inst)
{
  int32_t inst_idx = -1;
  for (size_t i = 0; i < _inst_list.size(); i++) {
...
void TetrisCluster::eraseTargetInstByIdx(int32_t idx){
  _inst_list.erase(_inst_list.begin() + idx);
}

void TetrisCluster::eraseTargetInstByIdxPair(int32_t begin_idx, int32_t end_idx){
