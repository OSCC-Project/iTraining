...
// ***************************************************************************************
#include "Log.hh"
#include "TetrisLegalizer.hh"
#include "usage/usage.hh"
#include "utility/Utility.hh"

namespace ipl {

TetrisLegalizer& TetrisLegalizer::getInst()
{
  if (!_s_tetris_lg_instance) {
    _s_tetris_lg_instance = new TetrisLegalizer();
  }
  return *_s_tetris_lg_instance;
}

void TetrisLegalizer::destoryInst()
{
  if (_s_tetris_lg_instance) {
    delete _s_tetris_lg_instance;
  }
}
...
void TetrisLegalizer::initTetrisLegalizer(Config* pl_config, PlacerDB* placer_db)
{
...
void TetrisLegalizer::initLGConfig(Config* pl_config)
{
  _config = pl_config->get_lg_config();
}

void TetrisLegalizer::initLGDatabase(PlacerDB* placer_db)
{
...
void TetrisLegalizer::initLGLayout()
{
...
void TetrisLegalizer::wrapRowList()
{
  const Layout* pl_layout = _database._placer_db->get_layout();
...
void TetrisLegalizer::wrapRegionList()
{
...
void TetrisLegalizer::wrapCellList()
{
  const Layout* pl_layout = _database._placer_db->get_layout();
...
void TetrisLegalizer::updateInstanceList()
{
...
void TetrisLegalizer::updateInstanceList(std::vector<Instance*> inst_list)
{
  checkMapping();
...
bool TetrisLegalizer::checkMapping()
{
  bool flag = true;
...
LGInstance* TetrisLegalizer::findLGInstance(Instance* pl_inst)
{
...
bool TetrisLegalizer::checkInstChanged(Instance* pl_inst, LGInstance* lg_inst)
{
  bool flag = false;
...
void TetrisLegalizer::updateInstanceInfo(Instance* pl_inst, LGInstance* lg_inst)
{
...
void TetrisLegalizer::updateInstanceMapping(Instance* pl_inst, LGInstance* lg_inst)
{
...
void TetrisLegalizer::initSegmentList()
{
...
bool TetrisLegalizer::runLegalize()
{
...
bool TetrisLegalizer::runIncrLegalize()
{
...
bool TetrisLegalizer::runIncrementalMode()
{
  ...
  for (auto* inst : _target_inst_list) {
    ...
    placeRow(inst, row_avail, false);
  }

  return true;
}

void TetrisLegalizer::pickAndSortMovableInstList(std::vector<LGInstance*>& movable_inst_list)
{
  double max_weight = INT32_MIN;
  double min_weight = INT32_MAX;
  double ratio = 0.28;  //单元宽度权重
  for (auto* inst : _database._lgInstance_list) {
    if (inst->get_state() == LGINSTANCE_STATE::kFixed) {
      continue;
    }
    // 设置单元归一化权重
    max_weight = max_weight < (1 - ratio) * inst->get_coordi().get_x() - ratio * inst->get_shape().get_width()
                     ? (1 - ratio) * inst->get_coordi().get_x() - ratio * inst->get_shape().get_width()
                     : max_weight;
    min_weight = min_weight > (1 - ratio) * inst->get_coordi().get_x() - ratio * inst->get_shape().get_width()
                     ? (1 - ratio) * inst->get_coordi().get_x() - ratio * inst->get_shape().get_width()
                     : min_weight;
    movable_inst_list.push_back(inst);
  }
  for (auto* inst : movable_inst_list) {
    inst->set_weight((max_weight - (1 - ratio) * inst->get_coordi().get_x() + ratio * inst->get_shape().get_width())
                     / (max_weight - min_weight));
  }
  // 根据权重进行排序
  std::sort(movable_inst_list.begin(), movable_inst_list.end(),
            [](LGInstance* l_inst, LGInstance* r_inst) { return (l_inst->get_weight() > r_inst->get_weight()); });
}

int32_t TetrisLegalizer::placeRow(LGInstance* inst, int32_t row_idx, bool is_trial)
{
  ...
  int32_t interval_list_size = _database._lg_layout->get_interval_2d_list()[row_idx].size();
  int32_t interval_idx = INT32_MAX;
  // Determine clusters and their optimal positions x_c(c):
  for (int32_t i = 0; i < interval_list_size; ++i) {
    if (inst_shape.get_width() <= interval_list[i]->get_remain_length()) {
      interval_idx = i;
      break;
    }
  }
...
int32_t TetrisLegalizer::searchNearestIntervalIndex(std::vector<LGInterval*>& segment_list, Rectangle<int32_t>& inst_shape)
{
...
int32_t TetrisLegalizer::searchRemainSpaceSegIndex(std::vector<LGInterval*>& segment_list, Rectangle<int32_t>& inst_shape,
...
LGCluster TetrisLegalizer::arrangeInstIntoIntervalCluster(LGInstance* inst, LGInterval* interval)
{
...
void TetrisLegalizer::arrangeClusterMinXCoordi(LGCluster& cluster)
{
...
void TetrisLegalizer::legalizeCluster(LGCluster& cluster)
{
...
int32_t TetrisLegalizer::obtainFrontMaxX(LGCluster& cluster)
{
...
int32_t TetrisLegalizer::obtainBackMinX(LGCluster& cluster)
{
...
void TetrisLegalizer::replaceClusterInfo(LGCluster& modify_cluster)
{
...
int32_t TetrisLegalizer::calDistanceWithBox(int32_t min_x, int32_t max_x, int32_t box_min_x, int32_t box_max_x)
{
...
bool TetrisLegalizer::checkOverlapWithBox(int32_t min_x, int32_t max_x, int32_t box_min_x, int32_t box_max_x)
{
  if (max_x > box_min_x && min_x < box_max_x) {
    return true;
  } else {
...
void TetrisLegalizer::alignInstanceOrient(std::vector<LGInstance*> inst_list)
{
...
void TetrisLegalizer::writebackPlacerDB(std::vector<LGInstance*> inst_list)
{
...
int32_t TetrisLegalizer::calTotalMovement()
{
...
// private
TetrisLegalizer* TetrisLegalizer::_s_tetris_lg_instance = nullptr;
